...
const LangSwitcher: React.FC<{ dict: any }> = ({ dict }) => {
  ...
  const handleSelect = (newLocale: Locale) => {
    ...
    const cookieString = `NEXT_LOCALE=${newLocale}; path=/; max-age=${60 * 60 * 24 * 365}${cookieDomain ? `; domain=${cookieDomain}` : ''}`;
    ...
    <div className="relative h-fit group ">
      <div className="flex items-center cursor-pointer  lg:justify-between  lg:px-2 py-1 rounded-md gap-1 lg:border">
        ...
        <div
          ...
          className="rounded-md lg:hidden group-hover:block lg:p-2 right-0 absolute lg:bg-background top-full  lg:shadow-lg w-[90%] lg:w-[8em]  z-50"
        >
          <ul className="px-3">
