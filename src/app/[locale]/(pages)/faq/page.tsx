    ...
    <PageContainer id="faq" path="" title={dict.faq.pageTitle}>
      <div className="lg:py-20 lg:px-6 px-4 py-10 max-w-3xl mx-auto">
        <div className="text-center mb-16 flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-foreground/50">{dict.faq.subtext}</p>
        </div>
        
        <div className="flex flex-col gap-6">
          {dict.faq.sections.map((item: any, index: number) => (
            <div key={index} className="flex flex-col gap-2 p-6 bg-foreground/5 rounded-lg">
              <h3 className="text-lg font-bold text-foreground">{item.question}</h3>
              <p className="text-base text-foreground/70 whitespace-pre-wrap">
                {item.answer}
              </p>
            </div>
          ))}
