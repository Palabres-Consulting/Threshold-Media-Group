...
const Categories: React.FC<CategoriesProps> = ({
  ...
  const [openMobileIndex, setOpenMobileIndex] = useState<
    number | string | null
  >(null);
  ...
  const getActiveState = (
    ...
    if (isTransverse) {
      ...
      return (
        cleanPathname === "/transverse" ||
        cleanPathname.startsWith("/transverse/")
      );
    }
    ...
    if (index === 0 || index === 1) {
      ...
      return (
        clientHost === targetHost.replace(/^https?:\/\//, "").replace(/\/$/, "")
      );
    }
  ...
  const transverseSubCategories = [{ title: "AI or Die", slug: "ai-or-die" }];

  const isMainDomain =
    clientHost ===
    mainDomainLink.replace(/^https?:\/\//, "").replace(/\/$/, "");
  ...
  return (
    <div className="flex flex-col rounded-3xl lg:flex-row lg:flex-nowrap lg:items-center lg:p-1 lg:border-[1px] border-foreground/20 lg:gap-2 gap-5 w-full lg:w-auto">
      ...
      <Link
        href={mainDomainLink}
        className={`block flex-grow py-1 text-[#F15901] lg:px-4 lg:border-sub rounded-2xl transition-colors whitespace-nowrap hover:bg-foreground/10 ${isMainDomain ? "bg-[#F15901]/70 text-white" : ""}`}
      >
        <Home size={18} />
      </Link>
      ...
      {visibleCategories.map((category, index) => {
        ...
        const isSubdomain = index === 0 || index === 1 || index === 2; // Extraction, ASINT, Innovation are subdomains

        const linkStyles = `block flex-grow py-1 lg:px-4 lg:border-sub rounded-2xl transition-colors whitespace-nowrap ${
          linkActive ? "bg-accent-main/70 text-white" : "hover:bg-foreground/10"
        }`;
      ...
      <div className="relative group w-full lg:w-auto">
        <div className="flex items-center justify-between md:w-fit ">
          ...
          >
            <ChevronDown
              className={`transition-transform duration-200 ${isTransverseOpen ? "rotate-180" : ""}`}
              size={16}
            />
          </button>
        </div>
